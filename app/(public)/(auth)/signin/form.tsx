...
export const SignInForm = ({ email }: SignInFormProps) => {
  ...
  const [signInMFAformData, setSignInMFAFormData] = React.useState<
    z.infer<typeof signInWithMFASchema>
  >({
    code: "",
    isBackupCode: false,
    sessionId: "",
  ...
  const signinMutation = useMutation({
    mutationFn: async (input: z.infer<typeof signInSchema>) => {
      return await mainFetch.post<{
        message: string;
        session?: { sessionId: string; expires: string };
      }>("/auth/signin", input);
    },
    onSuccess({ data: { message, session } }) {
      if (session) {
  ...
  const signinMFAMutation = useMutation({
    mutationFn: async (input: z.infer<typeof signInWithMFASchema>) => {
